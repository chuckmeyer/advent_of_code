...
function isFree (row, column, seatMap) {
  return seatMap[row][column] === 'L'
}
...
function seeNeighbor (row, nextRow, column, nextColumn, seatMap) {
  if (inBounds(row, column, seatMap)) {
    if (isOccupied(row, column, seatMap)) {
      return true
    } else if (isFree(row, column, seatMap)) {
      return false
    } else {
      return seeNeighbor(row + nextRow, nextRow, column + nextColumn, nextColumn, seatMap)
    }
  } else {
    return false
  }
...
function findNeighbors (row, column, seatMap) {
  ...
  for (const i of [-1, 0, 1]) {
    for (const j of [-1, 0, 1]) {
      if (i !== 0 || j !== 0) {
        if (seeNeighbor(row + i, i, column + j, j, seatMap)) {
          occupied++
        }
      }
    }
    ...
    for (let j = 0; j < seatMap[i].length; j++) {
      if (seatMap[i][j] !== '.') {
        ...
        if (seatMap[i][j] === 'L' && neighbors < 1) {
          nextSeatMap[i][j] = '#'
        } else if (seatMap[i][j] === '#' && neighbors >= 5) {
          nextSeatMap[i][j] = 'L'
        }
      }
    }
